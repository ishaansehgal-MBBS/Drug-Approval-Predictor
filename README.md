...
* **Stratified Validation:** Employs a 70/30 train/test split and 5-fold `StratifiedKFold` cross-validation to maintain consistent target class distribution across splits[cite: 1].
* **Model Benchmarking & Tuning:** Trains and compares Logistic Regression and Random Forest Classifiers using `GridSearchCV` hyperparameter optimization[cite: 1].
* **Feature Explainability:** Extracts coefficient weights and tree importances to identify key structural drivers of clinical trial success (e.g., administration route, study design, sponsor tier)[cite: 1].

---

## Performance & Results

Given the class imbalance in historical trial outcomes (~18.7% baseline success rate)[cite: 1], performance is evaluated using Area Under the Receiver Operating Characteristic Curve (**ROC-AUC**):
...
1. **Oral Administration (`route.1`):** Positively impacts success probability over complex routes[cite: 1].
2. **Double-Blind Design (`dkw.13`):** High study design rigor strongly correlates with trial success[cite: 1].
3. **Non-Top 20 Pharma Sponsor (`stid.1`):** Specialized mid-tier pharma backing provides positive probability lift[cite: 1].
4. **Top 20 Pharma Sponsor (`stid.31`):** Institutional resource backing improves success odds[cite: 1].
5. **Biologics / Antibodies (`origin.25`):** Protein/antibody drug origins show structural advantages over traditional small molecules[cite: 1].
